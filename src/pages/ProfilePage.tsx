...
export function ProfilePage() {
    const { profile, saveProfile, loading, error, clearError, dashboard } = useFitness()
                        ...
                        <Box component="form" onSubmit={handleSubmit}>
                            <Stack spacing={2.5}>
                                ...
                                <Box className="flex flex-wrap gap-3 pt-2">
                                    <Button uiVariant="primary" type="submit" disabled={loading.profile}>
                                        {loading.profile ? 'Saving profile...' : 'Save profile'}
                                    </Button>
                                    <Button uiVariant="outlined" type="button" onClick={() => setForm(defaultProfile())}>
                                        Reset
                                    </Button>
                                </Box>
                            </Stack>
                ...
                <Grid size={{ xs: 12, lg: 5 }}>
                    <Stack spacing={3}>
                        <AIResponseCard
                            ...
                            badges={[profile?.goal || 'No goal yet', profile?.activityLevel || 'No activity level yet']}
                            content={profile
                        ...
                        <Card padding="lg">
                            <Stack spacing={1.5}>
                                ...
                                <Typography variant="body2" color="text.secondary">
                                    Profile data persists in the backend JSON store so it can be expanded into a database later without changing the UI contract.
                                </Typography>
                            </Stack>
                        </Card>
                    </Stack>
